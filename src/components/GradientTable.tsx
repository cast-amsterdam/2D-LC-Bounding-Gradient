...
export function GradientTable({ title, points, onChange }: Props) {
  ...
  return <section className="bg2dlc-table-card">
    <div className="bg2dlc-table-heading"><h3>{title}</h3><button type="button" className="bg2dlc-icon-button" onClick={() => onChange([...points, { time: points[points.length - 1].time + 1, phi: points[points.length - 1].phi }])}>+ row</button></div>
    <div className="bg2dlc-table-scroll"><table><thead><tr><th>Time (min)</th><th>%B</th><th aria-label="Actions" /></tr></thead><tbody>
      {points.map((point, index) => <tr key={index}><td><input aria-label={`${title} time row ${index + 1}`} type="number" min="0" step="any" value={point.time} onChange={(event) => update(index, "time", event.target.value)} /></td><td><input aria-label={`${title} percent B row ${index + 1}`} type="number" min="0" max="100" step="any" value={point.phi} onChange={(event) => update(index, "phi", event.target.value)} /></td><td><button type="button" className="bg2dlc-remove" disabled={points.length <= 2} onClick={() => onChange(points.filter((_, i) => i !== index))} aria-label={`Remove ${title} row ${index + 1}`}>×</button></td></tr>)}
    </tbody></table></div>
  </section>;
}
